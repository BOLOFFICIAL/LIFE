    ...
    public partial class Form1 : Form
    {
        ...
        private void Initialization() 
        {
            ...
            else 
            {
                ...
                ages = new int[columns, rows];
                for (int i = 0; i < columns; i++)
                {
                    for (int j = 0; j < rows; j++)
                    {
                        field[i, j] = random.Next(density) == 0;
                        ages[i, j] = random.Next(maxAge);
                        if (field[i, j]) 
                        {
                            alllife++;
                        }
        ...
        private void Generation() 
        {
            ...
            for (int i = 0; i < columns; i++)
            {
                for (int j = 0; j < rows; j++)
                {
                    if (ages[i, j] < maxAge/5*1) 
                    {
                        color = Brushes.Red;
                    }
                    if (ages[i, j] > maxAge / 5 * 1 && ages[i, j] < maxAge / 5 * 2)
                    {
                        color = Brushes.Green;
                    }
                    if (ages[i, j] > maxAge / 5 * 2 && ages[i, j] < maxAge / 5 * 3)
                    {
                        color = Brushes.Blue;
                    }
                    if (ages[i, j] > maxAge / 5 * 3 && ages[i, j] < maxAge / 5 * 4)
                    {
                        color = Brushes.Yellow;
                    }
                    if (ages[i, j] > maxAge / 5 * 4)
                    {
                        color = Brushes.Brown;
                    }
                    ...
                    else if (haslife && ((countneighbours < 2 || countneighbours > 3) || ages[i, j] > maxAge))
                    {
                        newfield[i, j] = false;
                        ages[i, j] = 0;
                    }
                    ...
                    if (haslife)
                    {
                        graphics.FillEllipse(color, i * resolution, j * resolution, resolution - 1, resolution - 1);
                        alllife++;
                        ages[i, j]++;
                    }
                }
